...
// tokenTypeString returns a human-readable string for a models.TokenType.
// Used in error messages and diagnostics.
func tokenTypeString(t models.TokenType) string {
	return t.String()
}
...
func (p *Parser) Parse(tokens []token.Token) (*ast.AST, error) {
	return p.parseInternal(convertLegacyTokens(tokens), nil)
}

// convertLegacyTokens converts []token.Token to []parserToken, resolving
// any unset ModelType fields via the string Type or literal keyword lookup.
func convertLegacyTokens(tokens []token.Token) []parserToken {
	internal := make([]parserToken, 0, len(tokens)*2)
	for _, t := range tokens {
		typ := t.ModelType
		if typ == 0 && t.Type != "" {
			// Resolve from string-based Type field (handles operators, punctuation, etc.)
			typ = t.Type.ToModelType()
		}
		if (typ == models.TokenTypeIdentifier || typ == models.TokenTypeKeyword) && t.Literal != "" {
			// The old converter may have left keywords as Identifier or generic Keyword;
			// try to resolve to a more specific keyword type
			if kwType := identifierToKeywordType(t.Literal); kwType != 0 {
				typ = kwType
			}
		}

		// Expand compound tokens (e.g. GROUPING SETS, ORDER BY) into separate parser tokens
		pt := parserToken{Type: typ, Literal: t.Literal}
		if expanded := expandCompoundToken(models.TokenWithSpan{
			Token: models.Token{Type: typ, Value: t.Literal},
		}); len(expanded) > 0 {
			internal = append(internal, expanded...)
		} else {
			internal = append(internal, pt)
		}
	}
	return internal
}

// ParseDirect parses tokenizer output directly without the legacy token.Token conversion.
// This is the recommended parsing method for new code using models.TokenWithSpan.
func (p *Parser) ParseDirect(tokens []models.TokenWithSpan) (*ast.AST, error) {
	internal, positions := convertTokensToInternal(tokens)
	return p.parseInternal(internal, positions)
}

// parseInternal is the core parsing loop shared by all Parse* methods.
func (p *Parser) parseInternal(tokens []parserToken, positions []TokenPosition) (*ast.AST, error) {
	p.tokens = tokens
...
func (p *Parser) ParseWithPositions(result *ConversionResult) (*ast.AST, error) {
	internal := convertLegacyTokens(result.Tokens)
	for i := range internal {
		if i < len(result.PositionMapping) {
			internal[i].Start = result.PositionMapping[i].Start
			internal[i].End = result.PositionMapping[i].End
		}
	}
	return p.parseInternal(internal, result.PositionMapping)
}
...
func (p *Parser) peekToken() parserToken {
...
// =============================================================================
// Type-based Helper Methods (Phase 2 - Direct Int Comparisons)
// =============================================================================

// isType checks if the current token's Type matches the expected type.
// Direct O(1) int comparison — no fallback path needed.
...
func (p *Parser) isIdentifier() bool {
	return p.currentToken.Type == models.TokenTypeIdentifier || p.currentToken.Type == models.TokenTypeDoubleQuotedString
}

// matchType checks if the current token's Type matches and advances if true.
...
// isQuantifier checks if the current token is ANY or ALL.
...
// isBooleanLiteral checks if the current token is TRUE or FALSE.
...
func (p *Parser) expectedError(expected string) error {
	return goerrors.ExpectedTokenError(expected, tokenTypeString(p.currentToken.Type), p.currentLocation(), "")
}

// parseIdent parses an identifier. Accepts regular identifiers, double-quoted strings,
// and non-reserved keywords that can be used as identifiers in context.
...
// (isNonReservedKeyword defined below)
...
// isStringLiteral checks if the current token is any string literal type.
func (p *Parser) isStringLiteral() bool {
	switch p.currentToken.Type {
	case models.TokenTypeString, models.TokenTypeSingleQuotedString,
		models.TokenTypeDoubleQuotedString, models.TokenTypeTripleSingleQuotedString,
		models.TokenTypeTripleDoubleQuotedString, models.TokenTypeDollarQuotedString,
		models.TokenTypeByteStringLiteral, models.TokenTypeNationalStringLiteral,
		models.TokenTypeEscapedStringLiteral, models.TokenTypeUnicodeStringLiteral,
...
// isIntLiteral checks if the current token is an integer literal (number without decimal).
func (p *Parser) isIntLiteral() bool {
	return p.currentToken.Type == models.TokenTypeNumber
}

// isFloatLiteral checks if the current token is a float literal (number with decimal/exponent).
func (p *Parser) isFloatLiteral() bool {
	return p.currentToken.Type == models.TokenTypeFloatLiteral
}
...
func (p *Parser) isNonReservedKeyword() bool {
	...
	case models.TokenTypeTarget, models.TokenTypeSource, models.TokenTypeMatched,
		models.TokenTypeKey, models.TokenTypeReplace,
		models.TokenTypeRestart, models.TokenTypeContinue, models.TokenTypeIdentity,
		models.TokenTypeRename, models.TokenTypeTo,
		models.TokenTypeFilter, models.TokenTypeWindow, models.TokenTypePartition,
		models.TokenTypeRows, models.TokenTypeRange, models.TokenTypeRow,
		models.TokenTypeFirst, models.TokenTypeLast, models.TokenTypeNext,
		models.TokenTypeOnly, models.TokenTypeTies, models.TokenTypePercent,
		models.TokenTypeNulls, models.TokenTypeRole, models.TokenTypeSets,
		models.TokenTypeGrouping, models.TokenTypeRollup, models.TokenTypeCube,
		models.TokenTypePassword, models.TokenTypeLogin,
		models.TokenTypeSuperuser, models.TokenTypeNoSuperuser,
		models.TokenTypeCreateDB, models.TokenTypeNoCreateDB,
		models.TokenTypeCreateRole, models.TokenTypeNoCreateRole,
		models.TokenTypeNoLogin, models.TokenTypeInterval, models.TokenTypeWithin:
		return true
	case models.TokenTypeKeyword:
		// Generic keywords - check literal value
		upper := strings.ToUpper(p.currentToken.Literal)
		switch upper {
		case "TARGET", "SOURCE", "MATCHED", "VALUE", "NAME", "TYPE", "STATUS",
			"CONNECTOR", "DCPROPERTIES", "URL", "POLICY", "HASH", "MEMBER",
...
// matchKeyword checks if the current token's literal (case-insensitive) matches
// the given keyword string and advances if true. Used for keywords that don't
// have dedicated models.TokenType constants.
func (p *Parser) matchKeyword(keyword string) bool {
	if strings.EqualFold(p.currentToken.Literal, keyword) {
		p.advance()
		return true
	}
	return false
...
// isKeyword checks if the current token's literal (case-insensitive) matches the given keyword.
...
func (p *Parser) isJoinKeyword() bool {
	switch p.currentToken.Type {
	case models.TokenTypeJoin, models.TokenTypeInner, models.TokenTypeLeft,
