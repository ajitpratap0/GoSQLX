...
type tokenConverter struct {
	buffer  []token.Token
	typeMap map[models.TokenType]token.Type // token.Type used here — bridge code eliminated in #215 final phase
}
...
func (tc *tokenConverter) convertSingleToken(t models.TokenWithSpan) (token.Token, error) {
	...
	if t.Token.Type == models.TokenTypeKeyword {
		...
		return token.Token{
			Type:      token.Type(t.Token.Value), // token.Type used here — bridge code eliminated in #215 final phase
	...
	tokenType := token.Type(fmt.Sprintf("%v", t.Token.Type)) // token.Type used here — bridge code eliminated in #215 final phase
...
func getKeywordTokenTypeWithModel(value string) (token.Type, models.TokenType) { // token.Type used here — bridge code eliminated in #215 final phase
	var upper []byte
	n := len(value)
	if n <= 32 {
...
func buildTypeMapping() map[models.TokenType]token.Type { // token.Type used here — bridge code eliminated in #215 final phase
	return map[models.TokenType]token.Type{ // token.Type used here — bridge code eliminated in #215 final phase
