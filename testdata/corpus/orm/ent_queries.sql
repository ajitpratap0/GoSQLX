-- Ent-style: Auto-generated queries from Ent ORM
SELECT DISTINCT "users"."id", "users"."name", "users"."email", "users"."created_at" FROM "users" WHERE "users"."name" LIKE '%john%' ORDER BY "users"."id" ASC LIMIT 10;
...
SELECT "users"."id", "users"."name", "users"."email" FROM "users" WHERE "users"."age" >= 18 AND "users"."age" <= 65 AND "users"."status" IN ('active', 'pending') AND NOT ("users"."role" = 'banned') ORDER BY "users"."created_at" DESC LIMIT 20 OFFSET 40;

INSERT INTO "users" ("name", "email", "age", "status", "role", "created_at") VALUES ('Alice', 'alice@example.com', 30, 'active', 'user', '2024-01-15T10:00:00Z') RETURNING "id";
